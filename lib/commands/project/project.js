          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            async.each(filteredResult, function(file, cb) {
              ...
              if (file.name === '.env') {
                ...
                //console.log("hi");
                fs.outputFile(path.join(directory, '/test/api/client', file.name), file.test);
              }
            }, function(err) {
              console.log(err);
            });
            /*filteredResult.forEach(function(file){
              //console.log(file);
              if(file.name == '/../schema/pets-{pet_id}.rb'){console.log(file.test);}
		            //fs.outputFile(path.join(directory,'/test/api/client', file.name), file.test.replace(/x-enum/g, "enum"), cb);
                if(file.name == '/../schema/pets-{pet_id}.rb'){console.log(file.name);}
              });*/
          });
        });
      });
...
function libClientGen(directory, options, cb) {
  ...
  findProjectFile(directory, null, function(err, projPath) {
    ...
    if (!fs.existsSync(path.join(directory, 'utilities'))) {
      fs.mkdirSync(path.join(directory, 'utilities'));
    }
          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            async.each(filteredResult, function(file, cb) {
              if (file.name === '.env') {
                ...
                fs.outputFile(path.join(directory, '/utilities', file.name), file.test);
              }
            }, function(err) {
              console.log(err);
            });
