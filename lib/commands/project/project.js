...
var LANGUAGE = ['js', 'rb'];
var CLIENT_GEN_OPTS = ['all', 'public', 'utilities', 'api']
var FRAMEWORKS = {
...
module.exports = {
  ...
  generateSchema: schemaGenerate,

  // for generating client files
  clientGen: clientGen
};
...
function testGenerate(directory, options, cb) {
  ...
  findProjectFile(directory, null, function(err, projPath) {
    ...
    if (!fs.existsSync(path.join(directory, 'api/spec/api/client'))) {
      fs.mkdirSync(path.join(directory, 'api/spec/api/client'));
    }
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var existingFiles = fs.readdirSync(path.join(directory, 'api/spec/api/client'));
          var skipAll = false;
          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            async.each(filteredResult, function(file, cb) {
              if (file.name === '.env') {
                ...
                fs.outputFile(path.join(directory, '/api/spec/api/client', file.name), file.test);
              }
            }, function(err) {
              console.log(err);
            });
...
function schemaGenerate(directory, options, cb) {
  var pathList = [];
  var desiredPaths = [];
  var testModule = options.testModule || TEST_MODULES[0];
  var assertionFormat = options.assertionFormat || TEST_ASSERTION_TYPES[0];
  ...
  findProjectFile(directory, null, function(err, projPath) {
    ...
    if (!fs.existsSync(path.join(directory, 'api/spec/api/schema'))) {
      fs.mkdirSync(path.join(directory, 'api/spec/api/schema'));
    }
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var finalResult = template.schemaGen(result, config);
          var existingFiles = fs.readdirSync(path.join(directory, 'api/spec/api/schema'));
          var skipAll = false;
          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            async.each(filteredResult, function(file, cb) {
              if (file.name === '.env') {
                ...
                fs.outputFile(path.join(directory, '/api/spec/api/schema', file.name), file.test);
              }
            }, function(err) {
              console.log(err);
            });
...
function libClientGen(directory, options, cb) {
  var pathList = [];
  var desiredPaths = [];
  var testModule = options.testModule || TEST_MODULES[0];
  var assertionFormat = options.assertionFormat || TEST_ASSERTION_TYPES[0];
  ...
  findProjectFile(directory, null, function(err, projPath) {
    ...
    if (!fs.existsSync(path.join(directory, 'api/utilities'))) {
      fs.mkdirSync(path.join(directory, 'api/utilities'));
    }
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var finalResult = template.libClientGen(config);
          var existingFiles = fs.readdirSync(path.join(directory, 'api/utilities'));
          var skipAll = false;
          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            async.each(filteredResult, function(file, cb) {
              if (file.name === '.env') {
                ...
                fs.outputFile(path.join(directory, '/api/utilities', file.name), file.test);
              }
            }, function(err) {
              console.log(err);
            });
...
function publicClientGen(directory, options, cb) {
  var pathList = [];
  var desiredPaths = [];
  var testModule = options.testModule || TEST_MODULES[0];
  var assertionFormat = options.assertionFormat || TEST_ASSERTION_TYPES[0];
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var finalResult = template.pubClientGen(config);
          var existingFiles = fs.readdirSync(path.join(directory, 'api/utilities/'));
          var skipAll = false;
...
function ApiClientGen(directory, options, cb) {
  var pathList = [];
  var desiredPaths = [];
  var testModule = options.testModule || TEST_MODULES[0];
  var assertionFormat = options.assertionFormat || TEST_ASSERTION_TYPES[0];
  ...
  findProjectFile(directory, null, function(err, projPath) {
    ...
    if (!fs.existsSync(path.join(directory, 'api/public_api/'))) {
      fs.mkdirSync(path.join(directory, 'api/public_api/'));
    }
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var finalResult = template.clientGen(result, config);
          var existingFiles = fs.readdirSync(path.join(directory, 'api/public_api/'));
          var skipAll = false;
          ...
          async.filterSeries(finalResult, function(file, cb) {
            ...
            async.each(filteredResult, function(file, cb) {
              if (file.name === '.env') {
                ...
                fs.outputFile(path.join(directory, '/api/public_api/', file.name), file.test);
              }
            }, function(err) {
              console.log(err);
            });
...
function clientGen(directory, options, cb) {
    switch(options.client) {
        case 'all':
            libClientGen(directory, options, cb);
            ApiClientGen(directory, options, cb);
        case 'public':
            publicClientGen(directory, options, cb);
            break;
        case 'utilities':
            libClientGen(directory, options, cb);
            break;
        case 'api':
            ...
            break;
        default:
            console.log("Please specify a value from the list : " + client_opts.join('|'));
    }
}
