...
module.exports = {
  ...
  // for generating httparty lib files
  libClientGen: libClientGen,

  //for generating public-api-client files
  pubClientGen: pubClientGen,

  //for generating client api classes for every operation
  clientGen: clientGen
};
...
function pubClientGen(directory, options, cb) {
  var pathList = [];
  var desiredPaths = [];
  var testModule = options.testModule || TEST_MODULES[0];
  var assertionFormat = options.assertionFormat || TEST_ASSERTION_TYPES[0];
    ...
    readProject(directory, options, function(err, project) {
      ...
      swaggerSpec.validateSwagger(project.api.swagger, options, function(err) {
        spec.resolve(project.api.swagger, function(err, result) {
          ...
          var existingFiles = fs.readdirSync(path.join(directory, '/utilities/'));
          var skipAll = false;
